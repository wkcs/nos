...
static ssize_t tft_lcd_buf_write(struct device *dev, addr_t pos, const void *buffer, size_t size)
{
...
static int zj_tft_lcd_init(void)
{
    struct tft_lcd *lcd;

    lcd = kmalloc(sizeof(struct tft_lcd), GFP_KERNEL);
    if (lcd == NULL) {
