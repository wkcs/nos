...
# -*- coding: utf-8 -*-
"""
改进的 Kconfig 解析器，支持完整的依赖关系和互斥处理
...
class KconfigParser:
    ...
    def __init__(self, root_dir: str = '.'):
        ...
        self.current_menu: Optional[KconfigMenu] = None
        self.menu_stack: List[KconfigMenu] = []
    ...
    def _parse_line(self, line: str):
        ...
        if stripped.startswith('choice'):
            ...
            if self.current_menu:
                self.current_menu.add_item(self.current_choice)
            return
        ...
        if stripped.startswith('config '):
            ...
            if self.current_menu:
                self.current_menu.add_item(self.current_symbol)
            return
    ...
    def evaluate_condition(self, condition: str, config: Dict[str, str]) -> bool:
        ...
        if ' && ' in condition:
            ...
            return all(self.evaluate_condition(part.strip(), config) for part in parts)
        
        if ' || ' in condition:
            parts = condition.split(' || ')
            return any(self.evaluate_condition(part.strip(), config) for part in parts)
        
        # 处理括号
        if condition.startswith('(') and condition.endswith(')'):
            return self.evaluate_condition(condition[1:-1], config)
    ...
    def update_visibility(self, config: Dict[str, str]):
        """更新符号可见性"""
        for name, symbol in self.symbols.items():
            symbol.visible = True
            for depends in symbol.depends:
                if not self.evaluate_condition(depends, config):
                    symbol.visible = False
                    break
                    
    def resolve_choices(self, config: Dict[str, str]):
        """解决选择组的互斥关系"""
        for choice in self.choices:
            ...
            if selected_count > 1:
                for option in choice.options:
                    config[option] = 'n'
                if selected_option:
                    config[selected_option] = 'y'
                    
            # 如果没有选项被选中，使用默认值
            elif selected_count == 0 and choice.default:
                default_option = choice.default.strip()
                if default_option in choice.options:
                ...
                if config.get(name) == 'y':
                    for select in symbol.selects:
                        if config.get(select) != 'y':
                            # 检查被选择的符号是否可见
                            selected_symbol = self.symbols.get(select)
                            if selected_symbol and selected_symbol.visible:
                                config[select] = 'y'
                                changed = True
    ...
    def validate_config(self, config: Dict[str, str]) -> List[str]:
        ...
        # 检查依赖关系
        for name, symbol in self.symbols.items():
            if config.get(name) == 'y' and not symbol.visible:
                errors.append(f"Symbol {name} is enabled but dependencies are not met")
                
        # 检查选择组
        for choice in self.choices:
            selected_options = [opt for opt in choice.options if config.get(opt) == 'y']
            if len(selected_options) > 1:
                errors.append(f"Multiple options selected in choice: {selected_options}")
...
def main():
    if len(sys.argv) < 4:
        print("Usage: kconfig_improved.py <root_dir> <defconfig> <output_dir>")
        sys.exit(1)
